...
=head2 C<openapi_overlay>

A JSON "overlay" to the spec can be passed in, as C<openapi_overlay>. This
will be merged into the spec after the main spec is either loaded,
or read from the backend with C<read_schema>.
